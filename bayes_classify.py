					...
					if(getattr(mean,'__iter__',False)):
						for i in range (0,len(mean)):
							if variance[i]>0:
								...
								probability=10**200*len(means)*(count)*(1/(stdev*2*math.pi))*math.exp(-(track_value[i]-mean[i])**2/(2*stdev))
								probabilities.append(probability)
					else:	
						if variance>0:
							stdev=math.sqrt(variance)
							# len(means) is to scale up so probability doesn't get infinitesimally small
							probability=10**200*len(means)*(count)*(1/(stdev*2*math.pi))*math.exp(-(track_value-mean)**2/(2*stdev))
							probabilities.append(probability)
